...
{% macro normalize_priority(col) %}
    case
        -- Nhóm 1 — đã là số. Chỉ nhận nếu NẰM TRONG miền 1..4 của contract;
        -- '0', '5', '-1' là số nhưng ngoài miền nên rơi xuống else.
        when try_cast(trim({{ col }}) as integer) between 1 and 4
            then try_cast(trim({{ col }}) as integer)

        -- Nhóm 2 — schema evolution từ 2026-08-10: nguồn đổi cách biểu diễn
        -- sang nhãn chữ, ý nghĩa không đổi. Quy về số theo tài liệu API.
        ...
        -- Nhóm 3 — dữ liệu lỗi thật. NULL là TÍN HIỆU "không hợp lệ";
...
{% macro priority_reject_reason(col) %}
    case
        when {{ col }} is null          then 'priority NULL — nguồn không gửi giá trị'
        when trim({{ col }}) = ''       then 'priority rỗng — nguồn gửi chuỗi trắng'
        when try_cast(trim({{ col }}) as integer) is not null
            then 'priority là số nhưng ngoài miền 1..4 của contract'
        else 'priority là chuỗi lạ, không có trong bảng quy đổi (urgent/high/medium/low)'
    end
{% endmacro %}
